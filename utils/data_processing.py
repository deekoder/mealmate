...
def generate_unique_id(row):
    """Create a consistent unique ID for a meal row"""
...
def parse_meal_plan_to_dataframe(meal_plan_text):
    ...
    return _create_default_meal_plan_dataframe(meal_plan_text)

def _create_default_meal_plan_dataframe(meal_plan_text):
    """Create a default DataFrame when parsing fails"""
    default_data = []
...
def create_pivot_table(df):
    ...
    try:
        ...
        expected_columns = ['Day', 'Breakfast', 'Lunch', 'Dinner', 'Snack']
        available_columns = [col for col in expected_columns if col in pivot_df.columns]
...
def parse_grocery_list_to_dict(grocery_list_text):
    ...
    if len(categories) <= 1:
        categories = _try_alternative_category_extraction(grocery_list_text)
    
    # If still no structured categories, create basic ones based on line patterns
    if not categories:
        categories = _create_default_categories(lines)
    
    # Remove empty categories
    return {k: v for k, v in categories.items() if v}

def _try_alternative_category_extraction(grocery_list_text):
    """Try an alternative approach to extract categories from grocery list"""
    categories = {}
...
def _create_default_categories(lines):
    """Create default categories based on item keywords"""
    categories = {
        "Produce": [],
        "Grains & Bread": [],
        "Proteins": [],
